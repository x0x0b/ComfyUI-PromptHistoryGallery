...
- One-shot fixer: run `scripts/format.sh` (requires `pipx`, Node). It executes `pipx run --spec ruff==0.14.10 ruff format .`, `pipx run --spec ruff==0.14.10 ruff check --select I --fix .`, and `npx prettier@3.7.4 -w "web/**/*.{js,jsx,ts,tsx,css,scss,html,json}"`.
